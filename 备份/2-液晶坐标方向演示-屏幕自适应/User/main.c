...
void LCD_Direction_Show(void)
{
	...
	{	
	 ...
	 //不推荐使用其它模式显示文字	其它模式显示文字会有镜像效果			
	 //其中 6 模式为大部分液晶例程的默认显示方向  
		LCD->my_disp_driver->xx_GramScan ( i );
			
		sprintf(dispBuff,"o%d. X --->",i);
		LCD->my_disp_driver->xx_DispStringLine_EN(LINE(0),dispBuff);//沿X方向显示文字
			
		sprintf(dispBuff,"o%d.Y|V",i);	
		LCD->my_disp_driver->xx_DispString_EN_YDir(0,0,dispBuff);//沿Y方向显示文字
