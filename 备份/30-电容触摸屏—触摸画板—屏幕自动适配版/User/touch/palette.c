...
void Palette_Init(void)
{
  ...
  LCD->my_disp_driver->xx_GramScan(LCD->my_display->LCD_SCAN_MODE);

  /* 整屏清为白色 */
	LCD->my_disp_driver->xx_SetBackColor(CL_WHITE);
  LCD->my_disp_driver->xx_Clear(0,0,LCD->my_display->dp_max_width,LCD->my_display->dp_max_height);	
  ...
  brush.shape = LINE_SINGLE_PIXCEL;
	...
	LCD->my_disp_driver->xx_SetTextColor(brush.color);
...
void Touch_Button_Init(void)
{
  ...
  button[8].end_y = LCD->my_display->LCD_Y_LENGTH;
  ...
  button[17].end_y = LCD->my_display->LCD_Y_LENGTH;
  ...
  if(x>PALETTE_START_X && pre_x>PALETTE_START_X )
  {
    switch(brush->shape)  /*根据画刷参数描绘不同的轨迹*/
    {
      ...
      case LINE_SINGLE_PIXCEL:                 
            ...
            {      
              LCD->my_disp_driver->xx_DrawLine(pre_x,pre_y,x,y);
            } 
      ...
      case LINE_2_PIXCEL:
        ...
        /* 描绘2像素宽度的轨迹线 */
        LCD->my_disp_driver->xx_DrawLineCircle(pre_x,pre_y,x,y,1);
      ...
      case LINE_4_PIXCEL:
            ...
            LCD->my_disp_driver->xx_DrawLineCircle(pre_x,pre_y,x,y,2);
      ...
      case LINE_6_PIXCEL:
        ...
        LCD->my_disp_driver->xx_DrawLineCircle(pre_x,pre_y,x,y,3);
      ...
      case LINE_8_PIXCEL:
            ...
            LCD->my_disp_driver->xx_DrawLineCircle(pre_x,pre_y,x,y,4);
     ...
     case LINE_16_PIXCEL:
            ...
            LCD->my_disp_driver->xx_DrawLineCircle(pre_x,pre_y,x,y,8);
      ...
      case LINE_20_PIXCEL:
            ...
            LCD->my_disp_driver->xx_DrawLineCircle(pre_x,pre_y,x,y,10);
      ...
      case LINE_WITH_CIRCLE:  
            ...
            {      
              LCD->my_disp_driver->xx_DrawLine(pre_x,pre_y,x,y);
              LCD->my_disp_driver->xx_DrawCircle(x,y,3,1);
            } 
      ...
      case RUBBER:
				...
				if(x-20<PALETTE_START_X ||						//画板左边界
					  x+20>LCD->my_display->LCD_X_LENGTH || x-20<0 || //液晶左右边界
						y+20>LCD->my_display->LCD_Y_LENGTH || y-20<0)	 //液晶上下边界				
					break;	
			
//        if(x>PALETTE_START_X+20)
        {
						LCD->my_disp_driver->xx_SetColors(CL_WHITE,CL_WHITE);
            
						LCD->my_disp_driver->xx_DrawRectangle( x-40/2,
                                y-40/2,
                                40,
                                40,1);   
...
static void Draw_Color_Button(void *btn)
{
  ...
  {
		/*背景为功能键相应的颜色*/
		LCD->my_disp_driver->xx_SetColors(ptr->para,CL_WHITE);
    LCD->my_disp_driver->xx_DrawRectangle(ptr->start_x,
                      ptr->start_y,
  ...
  else  /*按键按下*/
  {
    ...
    /*按钮边框*/
		LCD->my_disp_driver->xx_SetColors(CL_BLUE4,CL_WHITE);
    LCD->my_disp_driver->xx_DrawRectangle(ptr->start_x,
									ptr->start_y,
									ptr->end_x - ptr->start_x,
...
static void Draw_Clear_Button(void *btn)
{
  ...
  if(ptr->touch_flag == 0)
  {
		...
		LCD->my_disp_driver->xx_SetColors(CL_BUTTON_GREY,CL_WHITE);
    LCD->my_disp_driver->xx_DrawRectangle(ptr->start_x,
                      ptr->start_y,
                      ptr->end_x - ptr->start_x,
											ptr->end_y - ptr->start_y,1);
		...
		LCD->my_disp_driver->xx_SetColors(CL_RED,CL_BUTTON_GREY);
		/*选择字体，使用中英文显示时，尽量把英文选择成16*32的字体，
		*中文字体大小是32*32的，需要其它字体请自行制作字模*/
		/*这个函数只对英文字体起作用*/
		LCD->my_disp_driver->xx_SetFont(&Font16x32);
		
		LCD->my_disp_driver->xx_DispString_EN_CH( ptr->start_x + (ptr->end_x - ptr->start_x - 32*2 )/2,
  ...
  else  /*按键按下*/
  {
    ...
    LCD->my_disp_driver->xx_DrawRectangle(ptr->start_x,
		...
		LCD->my_disp_driver->xx_SetColors(CL_RED,CL_WHITE);
		/*选择字体，使用中英文显示时，尽量把英文选择成16*32的字体，
		*中文字体大小是32*32的，需要其它字体请自行制作字模*/
		/*这个函数只对英文字体起作用*/
		LCD->my_disp_driver->xx_SetFont(&Font16x32);
   ...
   uint16_t i;
    ...
    if(ptr->touch_flag ==0 )
    {
			...
			LCD->my_disp_driver->xx_DrawRectangle(ptr->start_x,
			 ...
			 /*显示文字时的背景颜色*/     
			LCD->my_disp_driver->xx_SetColors(CL_BLUE4,CL_BUTTON_GREY);
			LCD->my_disp_driver->xx_DrawRectangle(ptr->start_x,
										ptr->start_y,
										ptr->end_x - ptr->start_x,
										ptr->end_y - ptr->start_y,0);
	...
	LCD->my_disp_driver->xx_SetColors(CL_BLACK,CL_WHITE);	
  /*根据画刷形状描绘按钮图案*/
 switch(ptr->para)
  {
    ...
    case LINE_SINGLE_PIXCEL:      
				...
				LCD->my_disp_driver->xx_SetColors(CL_BLACK,CL_WHITE);
        LCD->my_disp_driver->xx_DrawLine(ptr->start_x+20,
    ...
    case LINE_2_PIXCEL:
        ...
        LCD->my_disp_driver->xx_DrawLineCircle(ptr->start_x+20,
    ...
    case LINE_4_PIXCEL:
     ...
     case LINE_WITH_CIRCLE: 
        LCD->my_disp_driver->xx_SetColors(CL_BLACK,CL_WHITE);      
        LCD->my_disp_driver->xx_DrawLine(ptr->start_x+5,
                          ptr->start_y+(ptr->end_y-ptr->start_y)/2,
                          ptr->end_x-5,
                          ptr->start_y+(ptr->end_y-ptr->start_y)/2);
        ...
        LCD->my_disp_driver->xx_DrawCircle(ptr->start_x+5+i*10,
                            ptr->start_y+(ptr->end_y-ptr->start_y)/2,
                            3,1);
    ...
    case RUBBER:
			LCD->my_disp_driver->xx_SetColors(CL_WHITE,CL_BLACK);
       LCD->my_disp_driver->xx_DrawRectangle( ptr->start_x+((ptr->end_x - ptr->start_x -40)/2),
...
static void Command_Select_Color(void *btn)
{
  ...
  brush.color = ptr->para;	
	LCD->my_disp_driver->xx_SetColors(brush.color,CL_WHITE);
...
{
		LCD->my_disp_driver->xx_SetColors(CL_WHITE,CL_WHITE);
    LCD->my_disp_driver->xx_DrawRectangle(PALETTE_START_X,
                      PALETTE_START_Y,                    
