...
TOUCH_PARAM_TypeDef GT917S_TP_Desc = 
{
	.tp_max_width = 800,
	.tp_max_height = 480,
	.config_reg_addr = 0x8450,
	.ic_id = GT917S,
	.config_talbe = CTP_CFG_GT917S,
	.config_talbe_len = CFG_GROUP_LEN(CTP_CFG_GT917S),
};

TOUCH_PARAM_TypeDef GT5688_TP_Desc = 
{
	.tp_max_width = 854,
	.tp_max_height = 480,
	.config_reg_addr = 0x8450,
	.ic_id = GT5688,
	.config_talbe = CTP_CFG_GT5688,
	.config_talbe_len = CFG_GROUP_LEN(CTP_CFG_GT5688),
};

DEVICE_PARAM_TypeDef *LCD_Device;
...
static void Goodix_TS_Work_Func(void)
{
    ...
    if (touch_num)
    {
        for (i = 0; i < touch_num; i++)						//一个点一个点处理
        {
            ...
            {
							if (LCD_Device->my_touchpanel->ic_id != GT917S )
							{
									/*根据扫描模式更正X/Y起始方向*/
								switch(LCD_Device->my_display->LCD_SCAN_MODE)
								{
									case 0:case 7:
										input_y  = LCD_Device->my_display->LCD_Y_LENGTH - input_y;
										break;
									
									case 2:case 3: 
										input_x  = LCD_Device->my_display->LCD_X_LENGTH - input_x;
										input_y  = LCD_Device->my_display->LCD_Y_LENGTH - input_y;
										break;
...
//                = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
    ...
    if(touchIC == GT9147)
		...
		{
			LCD_Device = &NT35510;
			cfg_info =  LCD_Device->my_touchpanel->config_talbe; //指向寄存器配置CTP_CFG_GT917S
			cfg_info_len = LCD_Device->my_touchpanel->config_talbe_len;//计算配置表的大小
		}
    ...
    memcpy(&config[GTP_ADDR_LENGTH], cfg_info, cfg_info_len);
		...
		/*根据LCD的扫描方向设置分辨率*/
		config[GTP_ADDR_LENGTH+1] = LCD_Device->my_display->LCD_X_LENGTH & 0xFF;
		config[GTP_ADDR_LENGTH+2] = LCD_Device->my_display->LCD_X_LENGTH >> 8;
		config[GTP_ADDR_LENGTH+3] = LCD_Device->my_display->LCD_Y_LENGTH & 0xFF;
		config[GTP_ADDR_LENGTH+4] = LCD_Device->my_display->LCD_Y_LENGTH >> 8;
...
free(config);	
    ...
    return LCD_Device->my_touchpanel->ic_id;
}
