...
void LCD_Test(void)
{
  ...
  LCD->my_disp_driver->xx_Clear(0,0,LCD->my_display->LCD_X_LENGTH,LCD->my_display->LCD_Y_LENGTH);	/* 清屏，显示全黑 */
	...
	/********显示字符串示例*******/ 
  LCD->my_disp_driver->xx_DispStringLine_EN_CH(LINE(0),"野火BH");
	//显示指定大小的字符 
	LCD->my_disp_driver->xx_DisplayStringEx(0,1*48,48,48,(uint8_t *)"野火BH",0);
	LCD->my_disp_driver->xx_DisplayStringEx(0,2*56,56,56,(uint8_t *)"野火BH",0);
