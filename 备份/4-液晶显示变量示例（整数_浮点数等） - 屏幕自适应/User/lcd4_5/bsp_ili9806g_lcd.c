...
//uint16_t ILI9806G_DP_Desc.LCD_X_LENGTH = ILI9806G_MORE_PIXEL;
//uint16_t ILI9806G_DP_Desc.LCD_Y_LENGTH = ILI9806G_LESS_PIXEL;
...
static uint32_t								ILI9806G_Read_Id							(void);
static void										LCD_SetFont										(sFONT *fonts);
static sFONT 									*LCD_GetFont									(void);
static void										ILI9806G_ClearLine						(uint16_t Line);
static void										LCD_SetBackColor							(uint16_t Color);
static void										LCD_SetTextColor							(uint16_t Color)	;
static void										LCD_SetColors									(uint16_t TextColor, uint16_t BackColor);
static void										LCD_GetColors									(uint16_t *TextColor, uint16_t *BackColor);

/* 描述一个显示屏，变量具体作用详见screen_dev.h */
DISP_PARAM_TypeDef ILI9806G_DP_Desc = 
{
	.dp_max_width				= 854,
	.dp_max_height			= 480,
	.device_id					= 0x5688,
	...
	.LCD_SCAN_MODE			= 6,
	.device_name				= "ILI9806G",
	.size_info					= "4.5",
};

/* LCD驱动接口，提供给上层应用，变量具体作用详见screen_dev.h */
DISP_DRIVE_PARAM_TypeDef ILI9806G_DP_Driver = 
{
	.xx_send_cmd								= NULL,
	.xx_write_data							= NULL,
	.xx_read_data								= NULL,
	.xx_init_func								= ILI9806G_Init,
	.xx_Read_Chipid							= ILI9806G_Read_Id,
	.xx_BackLed_Control					= ILI9806G_BackLed_Control,
	.xx_GramScan								= ILI9806G_GramScan,
	.xx_OpenWindow							= ILI9806G_OpenWindow,
	.xx_Clear										= ILI9806G_Clear,
	.xx_SetPointPixel						= ILI9806G_SetPointPixel,
	.xx_DrawLine								=	ILI9806G_DrawLine,    
	.xx_DrawRectangle						=	ILI9806G_DrawRectangle,      
	.xx_DrawCircle							=	ILI9806G_DrawCircle,      
	.xx_DrawLineCircle					=	ILI9806G_DrawLineCircle,		
	.xx_DispChar_EN							=	ILI9806G_DispChar_EN,    
	.xx_DispStringLine_EN				=	ILI9806G_DispStringLine_EN,   
	.xx_DispString_EN						=	ILI9806G_DispString_EN,
	.xx_DispString_EN_YDir 			=	ILI9806G_DispString_EN_YDir,
	.xx_DispChar_CH							=	ILI9806G_DispChar_CH,
	.xx_DispString_CH						=	ILI9806G_DispString_CH,
	.xx_DispString_EN_CH				=	ILI9806G_DispString_EN_CH,
	.xx_DispStringLine_EN_CH 		=	ILI9806G_DispStringLine_EN_CH,
	.xx_DispString_EN_CH_YDir		=	ILI9806G_DispString_EN_CH_YDir,					
	.xx_SetFont									=	LCD_SetFont,	
	.xx_GetFont									=	LCD_GetFont,
	.xx_ClearLine								=	ILI9806G_ClearLine,
	.xx_SetBackColor						=	LCD_SetBackColor,
	.xx_SetTextColor						=	LCD_SetTextColor,
	.xx_SetColors								=	LCD_SetColors,
	.xx_GetColors								=	LCD_GetColors,
	.xx_DisplayStringEx					=	ILI9806G_DisplayStringEx,
	.xx_DisplayStringEx_YDir		=	ILI9806G_DisplayStringEx_YDir,
};
/**
  * @brief  简单延时函数
  * @param  nCount ：延时计数值
...
/**
 * @brief  读ILI9806G的ID寄存器
 * @param  无
 * @retval 无
 */
#define delay do{i++;}while(i<0xff);
static uint32_t	ILI9806G_Read_Id	(void)
{
	__IO int i;
	__IO uint32_t Read_Id;
	ILI9806G_Write_Cmd(0xd3);        //ID寄存器地址
	delay;
	Read_Id=ILI9806G_Read_Data();    //预读取
	delay;
	Read_Id=ILI9806G_Read_Data();    //读取ID1
	delay;
	Read_Id=ILI9806G_Read_Data();    //读取ID2
	Read_Id<<=8;       
	Read_Id|=ILI9806G_Read_Data();	 //读取ID3
	delay;
	return Read_Id;
}
/**
...
void ILI9806G_Init ( void )
{
	...
	ILI9806G_GramScan(ILI9806G_DP_Desc.LCD_SCAN_MODE);
    
	ILI9806G_Clear(0,0,ILI9806G_DP_Desc.LCD_X_LENGTH,ILI9806G_DP_Desc.LCD_Y_LENGTH);	/* 清屏，显示全黑 */
...
{	
	...
	ILI9806G_DP_Desc.LCD_SCAN_MODE = ucOption;
	
	//根据模式更新XY方向的像素宽度
	if(ucOption%2 == 0)	
	{
		//0 2 4 6模式下X方向像素宽度为480，Y方向为854
		ILI9806G_DP_Desc.LCD_X_LENGTH = ILI9806G_LESS_PIXEL;
		ILI9806G_DP_Desc.LCD_Y_LENGTH =	ILI9806G_MORE_PIXEL;
	}
	else				
	{
		//1 3 5 7模式下X方向像素宽度为854，Y方向为480
		ILI9806G_DP_Desc.LCD_X_LENGTH = ILI9806G_MORE_PIXEL;
		ILI9806G_DP_Desc.LCD_Y_LENGTH =	ILI9806G_LESS_PIXEL; 
	}

	//0x36命令参数的高3位可用于设置GRAM扫描方向	
	ILI9806G_Write_Cmd ( 0x36 ); 
	ILI9806G_Write_Data (0x00 | (ucOption<<5));//根据ucOption的值设置LCD参数，共0-7种模式
	ILI9806G_Write_Cmd ( ILI934_CMD_SetCoordinateX ); 
	ILI9806G_Write_Data ( 0x00 );		/* x 起始坐标高8位 */
	ILI9806G_Write_Data ( 0x00 );		/* x 起始坐标低8位 */
	ILI9806G_Write_Data ( ((ILI9806G_DP_Desc.LCD_X_LENGTH-1)>>8)&0xFF ); /* x 结束坐标高8位 */	
	ILI9806G_Write_Data ( (ILI9806G_DP_Desc.LCD_X_LENGTH-1)&0xFF );				/* x 结束坐标低8位 */

	ILI9806G_Write_Cmd ( ILI934_CMD_SetCoordinateY ); 
	ILI9806G_Write_Data ( 0x00 );		/* y 起始坐标高8位 */
	ILI9806G_Write_Data ( 0x00 );		/* y 起始坐标低8位 */
	ILI9806G_Write_Data ( ((ILI9806G_DP_Desc.LCD_Y_LENGTH-1)>>8)&0xFF );	/* y 结束坐标高8位 */	 
	ILI9806G_Write_Data ( (ILI9806G_DP_Desc.LCD_Y_LENGTH-1)&0xFF );				/* y 结束坐标低8位 */

	/* write gram start */
	ILI9806G_Write_Cmd ( ILI934_CMD_SetPixel );	
}
...
{	
	ILI9806G_Write_Cmd ( ILI934_CMD_SetCoordinateX ); 				 /* 设置X坐标 */
	...
	ILI9806G_Write_Cmd ( ILI934_CMD_SetCoordinateY ); 			     /* 设置Y坐标*/
...
{	
	if ( ( usX < ILI9806G_DP_Desc.LCD_X_LENGTH ) && ( usY < ILI9806G_DP_Desc.LCD_Y_LENGTH ) )
  {
...
void ILI9806G_DispStringLine_EN (  uint16_t line,  char * pStr )
{
	...
	while ( * pStr != '\0' )
	{
		if ( ( usX - ILI9806G_DispWindow_X_Star + LCD_Currentfonts->Width ) > ILI9806G_DP_Desc.LCD_X_LENGTH )
		{
			usX = ILI9806G_DispWindow_X_Star;
			line += LCD_Currentfonts->Height;
		}
		
		if ( ( line - ILI9806G_DispWindow_Y_Star + LCD_Currentfonts->Height ) > ILI9806G_DP_Desc.LCD_Y_LENGTH )
		{
			usX = ILI9806G_DispWindow_X_Star;
...
void ILI9806G_DispString_EN ( 	uint16_t usX ,uint16_t usY,  char * pStr )
{
	while ( * pStr != '\0' )
	{
		...
		if ( ( usY - ILI9806G_DispWindow_Y_Star + LCD_Currentfonts->Height ) > ILI9806G_DP_Desc.LCD_Y_LENGTH )
		{
			usX = ILI9806G_DispWindow_X_Star;
...
{	
	...
	while( * pStr != '\0' )
	{		
		if ( ( usX - ILI9806G_DispWindow_X_Star + WIDTH_CH_CHAR ) > ILI9806G_DP_Desc.LCD_X_LENGTH )
		{
			usX = ILI9806G_DispWindow_X_Star;
			usY += HEIGHT_CH_CHAR;
		}
		
		if ( ( usY - ILI9806G_DispWindow_Y_Star + HEIGHT_CH_CHAR ) > ILI9806G_DP_Desc.LCD_Y_LENGTH )
		{
			usX = ILI9806G_DispWindow_X_Star;
			usY = ILI9806G_DispWindow_Y_Star;
		}	
...
void ILI9806G_DispStringLine_EN_CH (  uint16_t line, char * pStr )
{
	...
	while( * pStr != '\0' )
	{
		...
		else	                            //汉字字符
		{
			...
			if ( ( line - ILI9806G_DispWindow_Y_Star + HEIGHT_CH_CHAR ) > ILI9806G_DP_Desc.LCD_Y_LENGTH )
			{
				usX = ILI9806G_DispWindow_X_Star;
				line = ILI9806G_DispWindow_Y_Star;
			}	
...
static uint8_t zoomBuff[ZOOMMAXBUFF] = {0};	//用于缩放的缓存，最大支持到128*128
static uint8_t zoomTempBuff[1024] = {0};
...

{
	...
	{
			/****处理换行*****/
			if ( ( x - ILI9806G_DispWindow_X_Star + Charwidth ) > ILI9806G_DP_Desc.LCD_X_LENGTH )
			{
				x = ILI9806G_DispWindow_X_Star;
				y += Font_Height;
			}
			
			if ( ( y - ILI9806G_DispWindow_Y_Star + Font_Height ) > ILI9806G_DP_Desc.LCD_Y_LENGTH )
			{
				x = ILI9806G_DispWindow_X_Star;
																		 ...
																		 uint16_t DrawModel)  //是否反色显示
{
	...
	{			
			//统一使用汉字的宽高来计算换行
			if ( ( y - ILI9806G_DispWindow_X_Star + Font_width ) > ILI9806G_DP_Desc.LCD_X_LENGTH )
			{
				y = ILI9806G_DispWindow_X_Star;
				x += Font_width;
			}
			
			if ( ( x - ILI9806G_DispWindow_Y_Star + Font_Height ) > ILI9806G_DP_Desc.LCD_Y_LENGTH )
			{
				y = ILI9806G_DispWindow_X_Star;
...
{
  ILI9806G_Clear(0,Line,ILI9806G_DP_Desc.LCD_X_LENGTH,((sFONT *)LCD_GetFont())->Height);	/* 清屏，显示全黑 */
...
void ILI9806G_DrawLineCircle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,uint8_t thick )
{
  ...
  for (curpixel = 0; curpixel <= numpixels; curpixel++)
  {
		...
		//判断边界
		if(x+thick>ILI9806G_DP_Desc.LCD_X_LENGTH || x-thick<0 || //液晶左右边界
			y+thick>ILI9806G_DP_Desc.LCD_Y_LENGTH || y-thick<0  ) //液晶上下边界
			continue;
