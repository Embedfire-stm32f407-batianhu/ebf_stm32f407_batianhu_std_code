...
//uint16_t NT35510_DP_Desc.LCD_X_LENGTH = NT35510_MORE_PIXEL;
//uint16_t NT35510_DP_Desc.LCD_Y_LENGTH = NT35510_LESS_PIXEL;
...
static void										NT35510_ClearLine						(uint16_t Line);
...
DISP_PARAM_TypeDef NT35510_DP_Desc = 
{
	.dp_max_width				= 800,
	.dp_max_height			= 480,
	.device_id					= 0x5310,
...
DISP_DRIVE_PARAM_TypeDef NT35510_DP_Driver = 
{
	.xx_send_cmd								= NULL,
	.xx_write_data							= NULL,
	.xx_read_data								= NULL,
	.xx_init_func								= NT35510_Init,
	.xx_BackLed_Control					= NT35510_BackLed_Control,
	.xx_GramScan								= NT35510_GramScan,
	.xx_OpenWindow							= NT35510_OpenWindow,
	.xx_Clear										= NT35510_Clear,
	.xx_SetPointPixel						= NT35510_SetPointPixel,
	.xx_DrawLine								=	NT35510_DrawLine,    
	.xx_DrawRectangle						=	NT35510_DrawRectangle,      
	.xx_DrawCircle							=	NT35510_DrawCircle,      
	.xx_DrawLineCircle					=	NT35510_DrawLineCircle,		
	.xx_DispChar_EN							=	NT35510_DispChar_EN,    
	.xx_DispStringLine_EN				=	NT35510_DispStringLine_EN,   
	.xx_DispString_EN						=	NT35510_DispString_EN,
	.xx_DispString_EN_YDir 			=	NT35510_DispString_EN_YDir,
	.xx_DispChar_CH							=	NT35510_DispChar_CH,
	.xx_DispString_CH						=	NT35510_DispString_CH,
	.xx_DispString_EN_CH				=	NT35510_DispString_EN_CH,
	.xx_DispStringLine_EN_CH 		=	NT35510_DispStringLine_EN_CH,
	.xx_DispString_EN_CH_YDir		=	NT35510_DispString_EN_CH_YDir,					
	.xx_SetFont									=	LCD_SetFont,	
	.xx_GetFont									=	LCD_GetFont,
	.xx_ClearLine								=	NT35510_ClearLine,
	.xx_SetBackColor						=	LCD_SetBackColor,
	.xx_SetTextColor						=	LCD_SetTextColor,
	.xx_SetColors								=	LCD_SetColors,
	.xx_GetColors								=	LCD_GetColors,
	.xx_DisplayStringEx					=	NT35510_DisplayStringEx,
	.xx_DisplayStringEx_YDir		=	NT35510_DisplayStringEx_YDir,
};
...
{
	__IO int id1,id2,id3;
	NT35510_Write_Cmd(0x0400);
	id1 = NT35510_Read_Data();
	NT35510_Write_Cmd(0x0401);
	id2 = NT35510_Read_Data();
	NT35510_Write_Cmd(0x0402);
	id3 = NT35510_Read_Data();
}
/**
...
void NT35510_Init ( void )
{
	...
	NT35510_GramScan(NT35510_DP_Desc.LCD_SCAN_MODE);
    
	NT35510_Clear(0,0,NT35510_DP_Desc.LCD_X_LENGTH,NT35510_DP_Desc.LCD_Y_LENGTH);	/* 清屏，显示全黑 */
...
{	
	...
	NT35510_DP_Desc.LCD_SCAN_MODE = ucOption;
	
	//根据模式更新XY方向的像素宽度
	if(ucOption%2 == 0)	
	{
		//0 2 4 6模式下X方向像素宽度为480，Y方向为854
		NT35510_DP_Desc.LCD_X_LENGTH = NT35510_LESS_PIXEL;
		NT35510_DP_Desc.LCD_Y_LENGTH =	NT35510_MORE_PIXEL;
	}
	else				
	{
		//1 3 5 7模式下X方向像素宽度为854，Y方向为480
		NT35510_DP_Desc.LCD_X_LENGTH = NT35510_MORE_PIXEL;
		NT35510_DP_Desc.LCD_Y_LENGTH =	NT35510_LESS_PIXEL; 
	}
  ...
  NT35510_Write_Cmd ( CMD_SetCoordinateX + 2 ); 
	NT35510_Write_Data ( ((NT35510_DP_Desc.LCD_X_LENGTH-1)>>8)&0xFF ); /* x 结束坐标高8位 */	
  NT35510_Write_Cmd ( CMD_SetCoordinateX + 3 ); 
	NT35510_Write_Data ( (NT35510_DP_Desc.LCD_X_LENGTH-1)&0xFF );				/* x 结束坐标低8位 */
  ...
  NT35510_Write_Cmd ( CMD_SetCoordinateY + 2 ); 
	NT35510_Write_Data ( ((NT35510_DP_Desc.LCD_Y_LENGTH-1)>>8)&0xFF );	/* y 结束坐标高8位 */	 
  NT35510_Write_Cmd ( CMD_SetCoordinateY + 3 ); 
	NT35510_Write_Data ( (NT35510_DP_Desc.LCD_Y_LENGTH-1)&0xFF );				/* y 结束坐标低8位 */
...
{	
	if ( ( usX < NT35510_DP_Desc.LCD_X_LENGTH ) && ( usY < NT35510_DP_Desc.LCD_Y_LENGTH ) )
  {
...
void NT35510_DispStringLine_EN (  uint16_t line,  char * pStr )
{
	...
	while ( * pStr != '\0' )
	{
		if ( ( usX - NT35510_DispWindow_X_Star + LCD_Currentfonts->Width ) > NT35510_DP_Desc.LCD_X_LENGTH )
		{
			usX = NT35510_DispWindow_X_Star;
			line += LCD_Currentfonts->Height;
		}
		
		if ( ( line - NT35510_DispWindow_Y_Star + LCD_Currentfonts->Height ) > NT35510_DP_Desc.LCD_Y_LENGTH )
		{
			usX = NT35510_DispWindow_X_Star;
...
void NT35510_DispString_EN ( 	uint16_t usX ,uint16_t usY,  char * pStr )
{
	while ( * pStr != '\0' )
	{
		...
		if ( ( usY - NT35510_DispWindow_Y_Star + LCD_Currentfonts->Height ) > NT35510_DP_Desc.LCD_Y_LENGTH )
		{
			usX = NT35510_DispWindow_X_Star;
...
{	
	...
	while( * pStr != '\0' )
	{		
		if ( ( usX - NT35510_DispWindow_X_Star + WIDTH_CH_CHAR ) > NT35510_DP_Desc.LCD_X_LENGTH )
		{
			usX = NT35510_DispWindow_X_Star;
			usY += HEIGHT_CH_CHAR;
		}
		
		if ( ( usY - NT35510_DispWindow_Y_Star + HEIGHT_CH_CHAR ) > NT35510_DP_Desc.LCD_Y_LENGTH )
		{
			usX = NT35510_DispWindow_X_Star;
			usY = NT35510_DispWindow_Y_Star;
		}	
...
void NT35510_DispStringLine_EN_CH (  uint16_t line, char * pStr )
{
	...
	while( * pStr != '\0' )
	{
		...
		else	                            //汉字字符
		{
			...
			if ( ( line - NT35510_DispWindow_Y_Star + HEIGHT_CH_CHAR ) > NT35510_DP_Desc.LCD_Y_LENGTH )
			{
				usX = NT35510_DispWindow_X_Star;
				line = NT35510_DispWindow_Y_Star;
			}	
...

{
	...
	{
			/****处理换行*****/
			if ( ( x - NT35510_DispWindow_X_Star + Charwidth ) > NT35510_DP_Desc.LCD_X_LENGTH )
			{
				x = NT35510_DispWindow_X_Star;
				y += Font_Height;
			}
			
			if ( ( y - NT35510_DispWindow_Y_Star + Font_Height ) > NT35510_DP_Desc.LCD_Y_LENGTH )
			{
				x = NT35510_DispWindow_X_Star;
																		 ...
																		 uint16_t DrawModel)  //是否反色显示
{
	...
	{			
			//统一使用汉字的宽高来计算换行
			if ( ( y - NT35510_DispWindow_X_Star + Font_width ) > NT35510_DP_Desc.LCD_X_LENGTH )
			{
				y = NT35510_DispWindow_X_Star;
				x += Font_width;
			}
			
			if ( ( x - NT35510_DispWindow_Y_Star + Font_Height ) > NT35510_DP_Desc.LCD_Y_LENGTH )
			{
				y = NT35510_DispWindow_X_Star;
...
{
  NT35510_Clear(0,Line,NT35510_DP_Desc.LCD_X_LENGTH,((sFONT *)LCD_GetFont())->Height);	/* 清屏，显示全黑 */
...
/**
  ...
  * @retval None
  */
void NT35510_DrawLineCircle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,uint8_t thick )
{
  ...
  for (curpixel = 0; curpixel <= numpixels; curpixel++)
  {
		...
		//判断边界
		if(x+thick>NT35510_DP_Desc.LCD_X_LENGTH || x-thick<0 || //液晶左右边界
			y+thick>NT35510_DP_Desc.LCD_Y_LENGTH || y-thick<0  ) //液晶上下边界
			continue;
